    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4 md:px-0">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 glass-ultra p-2 rounded-2xl border border-white/5">
            <button
        ...
        <AnimatePresence mode="popLayout">
          {sortedExams.map((exam) => {
            ...
            return (
              ...
              >
                <GlassCard className={`relative overflow-hidden h-full flex flex-col group border-white/5 hover:border-primary/30 transition-all duration-500 ${isPast ? 'opacity-40 grayscale' : ''}`} tilt={true}>
                  <div className="flex justify-between items-start mb-6 relative z-10">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border border-white/10 ${
                      isPast ? 'bg-white/5 text-slate-500' : 
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <button onClick={() => handleShareWhatsApp(exam)} className="p-2.5 bg-white/5 hover:bg-white/10 text-slate-500 hover:text-[#25D366] rounded-xl transition-all">
                        <Share2 size={18} />
                      </button>
                      <button onClick={() => handleShareEmail(exam)} className="p-2.5 bg-white/5 hover:bg-white/10 text-slate-500 hover:text-primary rounded-xl transition-all">
                        <Mail size={18} />
                      </button>
                      {canManage && !isPast && (
                        <>
                          <button onClick={() => handleEdit(exam)} className="p-2.5 bg-white/5 hover:bg-white/10 text-slate-500 hover:text-primary rounded-xl transition-all">
                            <Edit2 size={18} />
                          </button>
                          <button onClick={() => handleDelete(exam.id)} className="p-2.5 bg-white/5 hover:bg-white/10 text-slate-500 hover:text-danger rounded-xl transition-all">
                            <Trash2 size={18} />
                          </button>
                        </>
                      )}
                  ...
                  <h3 className="text-xl font-black text-white mb-6 tracking-tight group-hover:text-primary transition-colors duration-500 relative z-10">{exam.subject}</h3>

                  <div className="space-y-4 flex-1 relative z-10">
                    <div className="flex items-center gap-4 text-sm text-slate-400 font-medium group-hover:text-slate-300 transition-colors">
                      <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-slate-500 group-hover:text-primary transition-colors border border-white/10">
                        <Calendar size={16} />
                      </div>
                    ...
                    {exam.notes && (
                      <div className="mt-4 p-4 rounded-2xl bg-white/5 border border-white/5 text-xs text-slate-500 italic font-medium leading-relaxed group-hover:border-primary/20 transition-all">
                  ...
                  <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between relative z-10">
                    ...
                    {!isPast && (
                      <div className="flex-1 ml-6 h-1 bg-white/5 rounded-full overflow-hidden">
                        <motion.div 
                          initial={{ width: 0 }}
      ...
      {sortedExams.length === 0 && (
        <div className="text-center py-24 glass-ultra rounded-[40px] border-2 border-dashed border-white/5 mx-4 md:mx-0">
          <Calendar size={64} className="mx-auto text-slate-800 mb-6" />
          <h3 className="text-xl font-black text-white tracking-tight">Aucun examen programmé</h3>
          <p className="text-slate-500 font-medium mt-2">Le calme avant la tempête...</p>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="space-y-8 p-2">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 ml-1">Date & Heure</label>
                <input 
                  type="datetime-local" 
                  required
                  value={formData.date}
                  onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                  className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl outline-none focus:ring-2 focus:ring-primary/50 transition-all text-white font-medium appearance-none"
                />
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 ml-1">Durée</label>
                <input 
                  type="text" 
                  required
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 ml-1">Localisation / Salle</label>
              <input 
                type="text" 
                required
                value={formData.room}
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 ml-1">Notes stratégiques</label>
              <textarea 
                rows={3}
