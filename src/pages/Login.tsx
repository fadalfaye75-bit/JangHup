...
export const Login: React.FC = () => {
 ...
 >
 <GlassCard className="rounded-[40px] border-[var(--glass-border)] shadow-2xl overflow-hidden"tilt={true}>
 ...
 className="w-20 h-20 bg-gradient-to-br from-primary to-neon-blue text-[var(--text-main)] rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-[0_0_40px_rgba(108,99,255,0.4)] relative group"
 >
 <GraduationCap size={40} />
 <motion.div 
 animate={{ scale: [1, 1.2, 1] }}
 transition={{ duration: 2, repeat: Infinity }}
 className="absolute -top-2 -right-2 w-7 h-7 bg-[var(--bg-card)] rounded-full border-4 border-[var(--bg-main)] flex items-center justify-center shadow-lg"
 >
 <Sparkles size={10} className="text-primary"/>
 </motion.div>
 </motion.div>

 <h1 className="text-4xl font-black text-[var(--text-main)] tracking-tighter mb-1">JàngHub</h1>
 <p className="text-[var(--text-secondary)] font-black uppercase tracking-[0.3em] text-[9px]">
 ...
 {mode !== 'register' && (
 <div className="flex bg-[var(--glass-bg)] p-1 rounded-2xl mb-8 border border-[var(--glass-border)] backdrop-blur-md">
 <button 
 onClick={() => setMode('class')}
 className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all duration-300 ${mode === 'class' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-[var(--text-secondary)] hover:text-[var(--text-main)]'}`}
 >
 Étudiant
 </button>
 <button 
 onClick={() => setMode('admin')}
 className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all duration-300 ${mode === 'admin' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-[var(--text-secondary)] hover:text-[var(--text-main)]'}`}
 >
 Admin
 </button>
 </div>
 ...
 <div className="absolute left-5 top-1/2 -translate-y-1/2 text-[var(--text-secondary)] group-focus-within:text-primary transition-colors duration-300">
 <Mail size={18} />
 </div>
 <input 
 type="email"
 ...
 className="text-[9px] font-black text-primary uppercase tracking-widest hover:text-[var(--text-main)] transition-colors duration-300"
 >
 Oublié ?
 </button>
 </div>
 ...
 className="w-20 h-20 bg-[var(--glass-bg)] rounded-[32px] flex items-center justify-center mx-auto text-primary border border-[var(--glass-border)] shadow-inner"
 >
 <ShieldCheck size={40} />
 </motion.div>
 ...
 className="text-primary font-black hover:text-[var(--text-main)] transition-colors ml-1"
 >
 S'inscrire
 </button>
 </p>
