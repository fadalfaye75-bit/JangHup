...
export const Meet: React.FC = () => {
 ...
 className="group"
 >
 <GlassCard className="flex flex-col h-full p-0 overflow-hidden border-[var(--glass-border)] hover:border-primary/30 transition-all duration-500"tilt={true}>
 <div className="p-8 flex justify-between items-start border-b border-[var(--glass-border)] relative z-10">
 <div className="w-16 h-16 rounded-2xl bg-[var(--glass-bg)] flex items-center justify-center p-4 border border-[var(--glass-border)] shadow-inner group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(108,99,255,0.2)] transition-all duration-500">
 ...
 <Video className="text-[var(--text-secondary)]"size={28} />
 )}
 </div>
 ...
 {canManage && (
 <>
 <button onClick={() => handleEdit(meet)} className="p-2.5 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-hover)] text-[var(--text-secondary)] hover:text-primary rounded-xl transition-all">
 <Edit2 size={18} />
 </button>
 <button onClick={() => handleDelete(meet.id)} className="p-2.5 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-hover)] text-[var(--text-secondary)] hover:text-danger rounded-xl transition-all">
 <Trash2 size={18} />
 </button>
 </>
 )}
 ...
 <h3 className="text-2xl font-black text-[var(--text-main)] mb-6 tracking-tight group-hover:text-primary transition-colors duration-500">{meet.title}</h3>
 
 <div className="space-y-4">
 <div className="flex items-center gap-4 text-[var(--text-secondary)] text-sm font-medium">
 <div className="w-10 h-10 rounded-xl bg-[var(--glass-bg)] flex items-center justify-center border border-[var(--glass-border)] shadow-inner">
 <CalendarIcon size={16} className="text-primary"/>
 </div>
 <div className="flex flex-col">
 {isFormatted ? (
 <>
 <span className="font-black text-[var(--text-main)] uppercase tracking-widest text-[10px] leading-none mb-1">{timeInfo.day}</span>
 <span className="text-[var(--text-secondary)] text-xs">{timeInfo.date}</span>
 </>
 ) : (
 <span>{meet.time}</span>
 )}
 ...
 <div className="flex flex-col">
 {isFormatted ? (
 <span className="font-black text-[var(--text-main)] text-lg tracking-tighter leading-none">{timeInfo.time}</span>
 ) : (
 <span>Horaire non défini</span>
 )}
 <div className="flex items-center gap-2 mt-1">
 <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse"/>
 <span className="text-[9px] font-black uppercase tracking-widest text-[var(--text-secondary)]">{meet.platform}</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <Monitor size={64} className="mx-auto text-[var(--text-secondary)] mb-6"/>
 <h3 className="text-xl font-black text-[var(--text-main)] tracking-tight">Aucune session active</h3>
 <p className="text-[var(--text-secondary)] font-medium mt-2">Le hub virtuel est actuellement en veille.</p>
 </div>
 )}
 ...
 <label className="block text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-3 ml-1">Titre de la réunion</label>
 <input 
 type="text"
 required
 ...
 <div className="space-y-3">
 <label className="block text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-3 ml-1">Plateforme</label>
 ...
 <div className="space-y-3">
 <label className="block text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-3 ml-1">Date et Heure</label>
 <input 
 type="datetime-local"
 required
 value={formData.time}
 onChange={(e) => setFormData({ ...formData, time: e.target.value })}
 className="w-full p-5 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-2xl outline-none focus:ring-2 focus:ring-primary/50 transition-all text-[var(--text-main)] font-medium"
 />
 </div>
