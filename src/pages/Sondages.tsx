...
const ProgressBar: React.FC<{ progress: number; isSelected?: boolean }> = ({ progress, isSelected }) => (
 <div className="w-full h-3 bg-[var(--glass-bg)] rounded-full overflow-hidden relative border border-[var(--glass-border)] shadow-inner">
 <motion.div 
 initial={{ width: 0 }}
 animate={{ width: `${progress}%` }}
 transition={{ duration: 1.5, ease: [0.22, 1, 0.36, 1] }}
 className={`h-full rounded-full relative ${isSelected ? 'bg-gradient-to-r from-primary to-neon-blue shadow-[0_0_20px_rgba(108,99,255,0.5)]' : 'bg-[var(--glass-bg-hover)]'}`}
 >
 {isSelected && (
 <motion.div 
 animate={{ x: ['-100%', '100%'] }}
...
export const Sondages: React.FC = () => {
 ...
 >
 <GlassCard className="p-8 glass-ultra border-[var(--glass-border)] shadow-[0_0_50px_rgba(0,0,0,0.5)]">
 <div className="flex justify-between items-center mb-10">
 <div className="space-y-1">
 <h2 className="text-3xl font-black text-[var(--text-main)] tracking-tight">Analyse des Données</h2>
 ...
 className="p-3 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-hover)] text-[var(--text-secondary)] hover:text-[var(--text-main)] rounded-2xl transition-all"
 >
 <X size={24} />
 </button>
 </div>
 ...
 <div className="text-center py-32 glass-ultra rounded-[48px] border-2 border-dashed border-[var(--glass-border)]">
 <BarChart2 size={80} className="mx-auto text-[var(--text-main)] mb-8"/>
 <h3 className="text-2xl font-black text-[var(--text-main)] tracking-tight">Aucun sondage actif</h3>
 <p className="text-[var(--text-secondary)] font-medium max-w-xs mx-auto mt-2">Revenez plus tard pour participer aux décisions collectives.</p>
 </div>
 ) : (
 ...
 >
 <GlassCard className="h-full flex flex-col p-8 border-[var(--glass-border)] hover:border-primary/30 transition-all duration-500 group"tilt={true}>
 ...
 className="p-2.5 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-hover)] text-[var(--text-secondary)] hover:text-[var(--text-main)] rounded-xl transition-all"
 ...
 <h3 className="text-2xl font-black text-[var(--text-main)] mb-10 leading-tight tracking-tight group-hover:text-primary transition-colors duration-500 relative z-10">
 {poll.question}
 </h3>
 ...
 : 'bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-secondary)] hover:border-[var(--glass-border)] hover:text-[var(--text-main)]'
 }`}
 >
 <div className="flex items-center gap-4 relative z-10">
 <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${
 isMyVote ? 'border-primary bg-primary shadow-[0_0_15px_rgba(108,99,255,0.5)]' : 'border-[var(--glass-border)] group-hover/opt:border-white/30'
 }`}>
 {isMyVote && <Check size={14} className="text-[var(--text-main)]"/>}
 </div>
 ...
 >
 <span className={`font-black text-lg tracking-tighter ${isMyVote ? 'text-primary' : 'text-[var(--text-secondary)]'}`}>
 {percentage}%
 </span>
 </motion.div>
 )}
 ...
 isMyVote ? 'bg-primary' : 'bg-[var(--bg-card)]'
 }`}
 />
 )}
 </button>
 ...
 <div className="mt-10 pt-8 border-t border-[var(--glass-border)] flex justify-between items-center relative z-10">
 ...
 className="px-12 py-5 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[24px] text-[10px] font-black uppercase tracking-[0.3em] text-[var(--text-main)] hover:bg-[var(--glass-bg-hover)] transition-all disabled:opacity-50 shadow-2xl"
 >
 ...
 className="w-full max-w-xl glass-ultra rounded-[40px] p-10 shadow-[0_0_100px_rgba(0,0,0,0.5)] border border-[var(--glass-border)]"
 >
 <div className="flex justify-between items-center mb-10">
 <div className="space-y-1">
 ...
 <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] ml-1">Question Centrale</label>
 <input 
 required
 value={newQuestion}
 onChange={(e) => setNewQuestion(e.target.value)}
 placeholder="Quelle est votre question ?"
 className="w-full p-5 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[24px] text-[var(--text-main)] outline-none focus:ring-2 focus:ring-primary/50 transition-all placeholder:text-[var(--text-secondary)] font-medium"
 />
 </div>

 <div className="space-y-4">
 <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] ml-1">Options de Réponse</label>
 <div className="space-y-4">
 ...
 className="flex-1 p-4 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[20px] text-[var(--text-main)] outline-none focus:ring-2 focus:ring-primary/50 transition-all placeholder:text-[var(--text-main)] font-medium"
 />
 {newOptions.length > 2 && (
 <button 
 type="button"
 ...
 className="w-full p-5 bg-[var(--glass-bg)] border-2 border-dashed border-[var(--glass-border)] rounded-[24px] text-[var(--text-secondary)] font-black text-[10px] uppercase tracking-[0.3em] hover:bg-[var(--glass-bg-hover)] hover:border-primary/30 hover:text-primary transition-all"
 >
 + Ajouter un vecteur
 </button>
 </div>
