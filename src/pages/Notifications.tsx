...
const NotificationItem = React.memo<{
 ...
 <div className="w-12 h-12 rounded-2xl bg-[var(--bg-secondary)] flex items-center justify-center shrink-0">
 {getIcon(notif.type)}
 </div>
 <div className="flex-1 min-w-0">
 <div className="flex justify-between items-start mb-1">
 <h3 className="font-bold text-[var(--text-main)]">{notif.title}</h3>
 <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest flex items-center gap-1">
 ...
 className="p-2 text-[var(--text-secondary)] hover:text-[#25D366] transition-colors"
 ...
 className="p-2 text-[var(--text-secondary)] hover:text-danger transition-colors"
 title="Supprimer"
 >
 <Trash2 size={18} />
 </button>
...
export const Notifications: React.FC = () => {
 ...
 <Btn variant="ghost"className="text-[var(--text-secondary)] hover:text-primary"onClick={handleMarkAllAsRead}>
 ...
 <div className="text-center py-20 bg-[var(--bg-card)] rounded-[2.5rem] border border-dashed border-[var(--glass-border)]">
 <Bell size={48} className="mx-auto text-[var(--text-muted)] mb-4"/>
 <p className="text-[var(--text-secondary)] font-medium">Vous n'avez aucune notification</p>
 </div>
 )}
 </div>
 </div>
