...
export const Announcements: React.FC = () => {
 ...
 className="text-[10px] font-black uppercase tracking-widest text-primary hover:text-[var(--text-main)] transition-colors"
 >
 Tout marquer lu
 </button>
 )}
 ...
 className="w-12 h-12 rounded-2xl bg-primary text-white flex items-center justify-center shadow-[0_0_20px_rgba(108,99,255,0.4)] hover:scale-110 transition-transform border border-[var(--glass-border)]"
 >
 <Plus size={24} />
 </button>
 )}
 ...
 >
 <GlassCard className={`p-0 overflow-hidden border-[var(--glass-border)] hover:border-primary/30 transition-all duration-500 group ${!isRead ? 'shadow-[0_0_30px_rgba(108,99,255,0.05)]' : ''}`} tilt={true}>
 {/* Card Header */}
 <div className="p-6 flex items-center justify-between border-b border-[var(--glass-border)] relative z-10">
 <div className="flex items-center gap-4">
 <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-primary to-accent p-[1px]">
 <div className="w-full h-full rounded-2xl bg-[#0F0F1A] flex items-center justify-center text-[var(--text-main)] font-black text-lg">
 {ann.author.charAt(0)}
 </div>
 </div>
 <div>
 <div className="flex items-center gap-2">
 <span className="font-black text-sm text-[var(--text-main)] tracking-tight">{ann.author}</span>
 ...
 <p className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.2em]">{ann.className}</p>
 </div>
 </div>
 ...
 className={`p-2.5 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-hover)] transition-all rounded-xl ${ann.isPinned ? 'text-primary' : 'text-[var(--text-secondary)] hover:text-primary'}`}
 >
 <Pin size={18} />
 </button>
 <button 
 ...
 <h3 className="text-xl font-black text-[var(--text-main)] leading-tight tracking-tight group-hover:text-primary transition-colors duration-500">
 {ann.title}
 </h3>
 <p className="text-sm text-[var(--text-secondary)] leading-relaxed whitespace-pre-wrap font-medium group-hover:text-[var(--text-main)] transition-colors">
 {ann.content}
 </p>
 </div>
 ...
 className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-primary hover:text-[var(--text-main)] transition-colors"
 >
 ...
 {/* Card Footer */}
 <div className="px-6 py-4 bg-[var(--glass-bg)] flex items-center justify-between border-t border-[var(--glass-border)] relative z-10">
 <div className="flex items-center gap-6">
 <button 
 onClick={() => handleMarkAsRead(ann.id)}
 className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-colors ${isRead ? 'text-[var(--text-secondary)]' : 'text-primary'}`}
 >
 ...
 <div className="text-center py-24 px-6 glass-ultra rounded-[40px] border-2 border-dashed border-[var(--glass-border)]">
 <div className="w-24 h-24 bg-[var(--glass-bg)] rounded-full flex items-center justify-center mx-auto mb-6 border border-[var(--glass-border)]">
 <Megaphone size={48} className="text-[var(--text-secondary)]"/>
 </div>
 <h3 className="text-xl font-black text-[var(--text-main)] tracking-tight">Aucune transmission</h3>
 <p className="text-[var(--text-secondary)] font-medium mt-2">Le canal de communication est actuellement silencieux.</p>
 </div>
 )}
 </div>
 ...
 <div className="space-y-6">
 <div>
 <label className="block text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-3 ml-1">Titre de l'annonce</label>
 <input 
 type="text"
 required
 ...
 <div>
 <label className="block text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-3 ml-1">Priorité</label>
 <select 
 value={formData.priority}
 onChange={(e) => setFormData({ ...formData, priority: e.target.value as any })}
 className="w-full p-5 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-2xl outline-none focus:ring-2 focus:ring-primary/50 transition-all text-[var(--text-main)] font-medium appearance-none cursor-pointer"
 >
 ...
 />
 <div className={`w-12 h-7 rounded-full transition-all duration-500 ${formData.isPinned ? 'bg-primary shadow-[0_0_15px_rgba(108,99,255,0.5)]' : 'bg-[var(--glass-bg-hover)]'}`} />
 <div className={`absolute top-1 left-1 w-5 h-5 bg-[var(--bg-card)] rounded-full transition-transform duration-500 ${formData.isPinned ? 'translate-x-5' : ''}`} />
 </div>
 <span className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest group-hover:text-primary transition-colors">Prioritaire</span>
 </label>
 </div>
 </div>

 <div>
 <label className="block text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-3 ml-1">Message</label>
 <textarea 
 required
 rows={5}
 ...
 <div>
 <label className="block text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-3 ml-1">Lien externe (optionnel)</label>
 <input 
 type="url"
 value={formData.link}
