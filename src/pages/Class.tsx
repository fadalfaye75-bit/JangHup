...
export const Class: React.FC = () => {
 ...
 <div className="w-20 h-20 bg-[var(--glass-bg)] rounded-full flex items-center justify-center mx-auto text-[var(--text-secondary)]">
 <Users size={40} />
 </div>
 ...
 className="w-full pl-14 pr-6 py-5 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[24px] outline-none focus:ring-2 focus:ring-primary/50 transition-all text-[var(--text-main)] placeholder:text-[var(--text-secondary)] font-medium"
 />
 </div>
 ...
 >
 <GlassCard className="p-5 border-[var(--glass-border)] hover:border-primary/30 transition-all duration-500 group"tilt={false}>
 <div className="flex items-center justify-between relative z-10">
 <div className="flex items-center gap-5">
 <div className="relative">
 <div className="w-14 h-14 rounded-2xl overflow-hidden border-2 border-[var(--glass-border)] shadow-2xl group-hover:border-primary/50 transition-all duration-500">
 <img 
 ...
 <div className="absolute -top-2 -right-2 w-6 h-6 bg-primary text-white rounded-lg flex items-center justify-center border-2 border-[var(--bg-card)] shadow-lg">
 <Shield size={12} />
 </div>
 )}
 </div>
 ...
 <h3 className="font-black text-[var(--text-main)] tracking-tight group-hover:text-primary transition-colors">{member.name} {member.id === user.id &&"(Moi)"}</h3>
 ...
 <p className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest flex items-center gap-2 mt-1">
 <Mail size={12} className="text-primary opacity-50"/>
 {member.email}
 </p>
 </div>
 </div>
 <button className="p-3 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-hover)] text-[var(--text-secondary)] hover:text-[var(--text-main)] rounded-xl transition-all opacity-0 group-hover:opacity-100">
 ...
 <div className="text-center py-20 glass-ultra rounded-[40px] border-2 border-dashed border-[var(--glass-border)]">
 <Users size={64} className="mx-auto text-[var(--text-main)] mb-6"/>
 <p className="text-[var(--text-secondary)] font-black uppercase tracking-widest text-xs">Aucun membre détecté dans ce secteur.</p>
 </div>
 )}
 </div>
 </div>
 ...
 <GlassCard className="p-8 space-y-8 border-[var(--glass-border)] hover:border-primary/30 transition-all duration-500"tilt={true}>
 ...
 <h2 className="text-xl font-black text-[var(--text-main)] tracking-tight">Accès Délégué</h2>
 </div>
 
 <p className="text-xs text-[var(--text-secondary)] font-medium leading-relaxed relative z-10">
 Le protocole délégué autorise l'accès aux outils de gestion stratégique du nexus.
 </p>

 <div className="p-6 bg-[var(--glass-bg)] rounded-[32px] border border-[var(--glass-border)] space-y-4 relative z-10">
 <div className="flex items-center justify-between">
 <span className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em]">Code de Liaison</span>
 {(user.role === UserRole.ADMIN || user.role === UserRole.DELEGATE) && (
 <button 
 onClick={handleGenerateNewCode}
 className="p-2 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-hover)] text-primary rounded-xl transition-all"
 title="Régénérer"
 >
 <RefreshCw size={14} />
 </button>
 ...
 <code className="text-3xl font-black text-[var(--text-main)] tracking-[0.2em] font-mono">
 {(user.role === UserRole.ADMIN || user.role === UserRole.DELEGATE) 
 ...
 <div className="pt-8 border-t border-[var(--glass-border)] relative z-10">
 <div className="flex items-center justify-between text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-6">
 <span>Métriques du Nexus</span>
 </div>
 <div className="grid grid-cols-2 gap-4">
 <div className="p-5 bg-[var(--glass-bg)] rounded-[24px] border border-[var(--glass-border)]">
 <p className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-widest mb-1">Étudiants</p>
 <p className="text-2xl font-black text-[var(--text-main)] tracking-tight">{members.filter(m => m.role === UserRole.STUDENT).length}</p>
 </div>
 <div className="p-5 bg-[var(--glass-bg)] rounded-[24px] border border-[var(--glass-border)]">
 <p className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-widest mb-1">Délégués</p>
 <p className="text-2xl font-black text-[var(--text-main)] tracking-tight">{members.filter(m => m.role === UserRole.DELEGATE).length}</p>
 </div>
 </div>
 </div>
 </GlassCard>

 <GlassCard className="p-8 bg-gradient-to-br from-primary to-accent text-[var(--text-main)] border-none shadow-[0_0_50px_rgba(108,99,255,0.3)] group overflow-hidden"tilt={true}>
 ...
 <button className="w-full py-5 bg-[var(--glass-bg-hover)] hover:bg-[var(--bg-card)]/20 rounded-[24px] font-black text-[10px] uppercase tracking-[0.3em] transition-all border border-[var(--glass-border)] backdrop-blur-md">
 ...
 <div className="p-8 bg-[var(--glass-bg)] rounded-[32px] border border-[var(--glass-border)] text-center space-y-6">
 ...
 <h3 className="text-2xl font-black text-[var(--text-main)] tracking-tight">Inviter des Unités</h3>
 ...
 <div className="space-y-4">
 <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] ml-1">Identifiant du Nexus</label>
 <div className="flex gap-3">
 <div className="flex-1 p-5 bg-[var(--glass-bg)] rounded-[24px] font-mono font-black text-[var(--text-main)] border border-[var(--glass-border)] tracking-widest">
 {user.class_name}
 </div>
 <button 
 onClick={() => {
 ...
 <button onClick={() => setIsInviteModalOpen(false)} className="w-full py-5 bg-[var(--glass-bg)] hover:bg-[var(--glass-bg-hover)] text-[var(--text-secondary)] hover:text-[var(--text-main)] rounded-[24px] font-black text-[10px] uppercase tracking-[0.3em] transition-all border border-[var(--glass-border)]">Fermer le Canal</button>
 </div>
 </Modal>
 </div>
 );
